Swal.fire({
  title: "Te amo Mi vida Eres Todo Para miiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii",
...
function heartPos(t, scale = 1) {
  ...
  return {
    x: canvas.width / (2 * (window.devicePixelRatio || 1)) + x * scale,
    y: canvas.height / (2 * (window.devicePixelRatio || 1)) + y * scale
  };
}
...
function drawStaticHeart() {
  ...
  ctx.fillText("Haz clic en el corazón 💖", canvas.width / (2 * (window.devicePixelRatio || 1)), canvas.height / (2 * (window.devicePixelRatio || 1)) + 80);
}

function isClickInsideHeart(x, y) {
  const dpr = window.devicePixelRatio || 1;
  return ctx.isPointInPath(heartPath, x * dpr, y * dpr);
}
